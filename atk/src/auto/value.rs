...
pub trait ValueExt: IsA<Value> + 'static {
    ...
    fn value_and_text(&self) -> (f64, glib::GString) {
        unsafe {
            ...
            ffi::atk_value_get_value_and_text(self.as_ref().to_glib_none().0, value.as_mut_ptr(), &mut text);
    ...
    fn connect_value_changed<F: Fn(&Self, f64, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn value_changed_trampoline<P: IsA<Value>, F: Fn(&P, f64, &str) + 'static>(this: *mut ffi::AtkValue, value: std::ffi::c_double, text: *mut std::ffi::c_char, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(Value::from_glib_borrow(this).unsafe_cast_ref(), value, &glib::GString::from_glib_borrow(text))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"value-changed\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(value_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
