...
impl TargetList {
    ...
    pub fn add_image_targets(&self, info: u32, writable: bool) {
        unsafe {
            ffi::gtk_target_list_add_image_targets(self.to_glib_none().0, info, writable.into_glib());
        }
    }
    ...
    pub fn add_rich_text_targets(&self, info: u32, deserializable: bool, buffer: &impl IsA<TextBuffer>) {
        unsafe {
            ffi::gtk_target_list_add_rich_text_targets(self.to_glib_none().0, info, deserializable.into_glib(), buffer.as_ref().to_glib_none().0);
        }
    }
    ...
    pub fn find(&self, target: &gdk::Atom) -> Option<u32> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_target_list_find(self.to_glib_none().0, target.to_glib_none().0, info.as_mut_ptr()));
