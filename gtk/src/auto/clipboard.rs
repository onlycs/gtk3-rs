...
impl Clipboard {
    ...
    pub fn request_contents<P: FnOnce(&Clipboard, &SelectionData) + 'static>(&self, target: &gdk::Atom, callback: P) {
        ...
        unsafe extern "C" fn callback_func<P: FnOnce(&Clipboard, &SelectionData) + 'static>(clipboard: *mut ffi::GtkClipboard, selection_data: *mut ffi::GtkSelectionData, data: glib::ffi::gpointer) {
        ...
        unsafe {
            ffi::gtk_clipboard_request_contents(self.to_glib_none().0, target.to_glib_none().0, callback, Box_::into_raw(super_callback0) as *mut _);
        }
    }

    #[doc(alias = "gtk_clipboard_request_image")]
    pub fn request_image<P: FnOnce(&Clipboard, Option<&gdk_pixbuf::Pixbuf>) + 'static>(&self, callback: P) {
        let callback_data: Box_<P> = Box_::new(callback);
        unsafe extern "C" fn callback_func<P: FnOnce(&Clipboard, Option<&gdk_pixbuf::Pixbuf>) + 'static>(clipboard: *mut ffi::GtkClipboard, pixbuf: *mut gdk_pixbuf::ffi::GdkPixbuf, data: glib::ffi::gpointer) {
            let clipboard = from_glib_borrow(clipboard);
        ...
        let super_callback0: Box_<P> = callback_data;
        unsafe {
            ffi::gtk_clipboard_request_image(self.to_glib_none().0, callback, Box_::into_raw(super_callback0) as *mut _);
        }
    }

    #[doc(alias = "gtk_clipboard_request_rich_text")]
    pub fn request_rich_text<P: FnOnce(&Clipboard, &gdk::Atom, Option<&str>, usize) + 'static>(&self, buffer: &impl IsA<TextBuffer>, callback: P) {
        let callback_data: Box_<P> = Box_::new(callback);
        unsafe extern "C" fn callback_func<P: FnOnce(&Clipboard, &gdk::Atom, Option<&str>, usize) + 'static>(clipboard: *mut ffi::GtkClipboard, format: gdk::ffi::GdkAtom, text: *const libc::c_char, length: libc::size_t, data: glib::ffi::gpointer) {
            let clipboard = from_glib_borrow(clipboard);
            let format = from_glib_borrow(format);
            let text: Borrowed<Option<glib::GString>> = from_glib_borrow(text);
            let callback: Box_<P> = Box_::from_raw(data as *mut _);
            (*callback)(&clipboard, &format, (*text).as_ref().map(|s| s.as_str()), length)
        }
        let callback = Some(callback_func::<P> as _);
        let super_callback0: Box_<P> = callback_data;
        unsafe {
            ffi::gtk_clipboard_request_rich_text(self.to_glib_none().0, buffer.as_ref().to_glib_none().0, callback, Box_::into_raw(super_callback0) as *mut _);
        }
    }
    ...
    pub fn request_text<P: FnOnce(&Clipboard, Option<&str>) + 'static>(&self, callback: P) {
        let callback_data: Box_<P> = Box_::new(callback);
        unsafe extern "C" fn callback_func<P: FnOnce(&Clipboard, Option<&str>) + 'static>(clipboard: *mut ffi::GtkClipboard, text: *const libc::c_char, data: glib::ffi::gpointer) {
            let clipboard = from_glib_borrow(clipboard);
            let text: Borrowed<Option<glib::GString>> = from_glib_borrow(text);
            let callback: Box_<P> = Box_::from_raw(data as *mut _);
        ...
        let super_callback0: Box_<P> = callback_data;
        unsafe {
            ffi::gtk_clipboard_request_text(self.to_glib_none().0, callback, Box_::into_raw(super_callback0) as *mut _);
        }
    }
    ...
    pub fn wait_for_rich_text(&self, buffer: &impl IsA<TextBuffer>) -> (Vec<u8>, gdk::Atom) {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_full_num(ffi::gtk_clipboard_wait_for_rich_text(self.to_glib_none().0, buffer.as_ref().to_glib_none().0, format.to_glib_none_mut().0, length.as_mut_ptr()), length.assume_init() as _);
            (ret, format)
        }
    }
    ...
    pub fn wait_for_targets(&self) -> Option<Vec<gdk::Atom>> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_clipboard_wait_for_targets(self.to_glib_none().0, &mut targets, n_targets.as_mut_ptr()));
            if ret { Some(FromGlibContainer::from_glib_container_num(targets, n_targets.assume_init() as _)) } else { None }
        }
    }
    ...
    #[allow(clippy::should_implement_trait)]    pub fn default(display: &gdk::Display) -> Option<Clipboard> {
        assert_initialized_main_thread!();
        unsafe {
