...
use crate::{Adjustment,Align,Buildable,Container,Orientable,Orientation,Range,SensitivityType,Widget};
...
impl Scrollbar {
    ...
    pub fn new(orientation: Orientation, adjustment: Option<&impl IsA<Adjustment>>) -> Scrollbar {
        ...
        unsafe {
            Widget::from_glib_none(ffi::gtk_scrollbar_new(orientation.into_glib(), adjustment.map(|p| p.as_ref()).to_glib_none().0)).unsafe_cast()
        }
    }
