...
pub trait TreeSelectionExt: IsA<TreeSelection> + sealed::Sealed + 'static {
    ...
    fn selected_foreach<P: FnMut(&TreeModel, &TreePath, &TreeIter)>(&self, func: P) {
        ...
        unsafe extern "C" fn func_func<P: FnMut(&TreeModel, &TreePath, &TreeIter)>(model: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, iter: *mut ffi::GtkTreeIter, data: glib::ffi::gpointer) {
    ...
    fn set_select_function(&self, func: Option<Box_<dyn Fn(&TreeSelection, &TreeModel, &TreePath, bool) -> bool + 'static>>) {
        let func_data: Box_<Option<Box_<dyn Fn(&TreeSelection, &TreeModel, &TreePath, bool) -> bool + 'static>>> = Box_::new(func);
        unsafe extern "C" fn func_func(selection: *mut ffi::GtkTreeSelection, model: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, path_currently_selected: glib::ffi::gboolean, data: glib::ffi::gpointer) -> glib::ffi::gboolean {
            let selection = from_glib_borrow(selection);
            let model = from_glib_borrow(model);
            let path = from_glib_borrow(path);
            let path_currently_selected = from_glib(path_currently_selected);
            let callback: &Option<Box_<dyn Fn(&TreeSelection, &TreeModel, &TreePath, bool) -> bool + 'static>> = &*(data as *mut _);
        ...
        unsafe extern "C" fn destroy_func(data: glib::ffi::gpointer) {
            let _callback: Box_<Option<Box_<dyn Fn(&TreeSelection, &TreeModel, &TreePath, bool) -> bool + 'static>>> = Box_::from_raw(data as *mut _);
        }
        let destroy_call3 = Some(destroy_func as _);
        let super_callback0: Box_<Option<Box_<dyn Fn(&TreeSelection, &TreeModel, &TreePath, bool) -> bool + 'static>>> = func_data;
        unsafe {
            ffi::gtk_tree_selection_set_select_function(self.as_ref().to_glib_none().0, func, Box_::into_raw(super_callback0) as *mut _, destroy_call3);
        }
    }
    ...
    fn connect_changed<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn changed_trampoline<P: IsA<TreeSelection>, F: Fn(&P) + 'static>(this: *mut ffi::GtkTreeSelection, f: glib::ffi::gpointer) {
    ...
    fn connect_mode_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_mode_trampoline<P: IsA<TreeSelection>, F: Fn(&P) + 'static>(this: *mut ffi::GtkTreeSelection, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(TreeSelection::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
