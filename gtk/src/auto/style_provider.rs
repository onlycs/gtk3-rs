...
pub trait StyleProviderExt: IsA<StyleProvider> + sealed::Sealed + 'static {
    ...
    fn style_property(&self, path: &WidgetPath, state: StateFlags, pspec: impl AsRef<glib::ParamSpec>) -> Option<glib::Value> {
        unsafe {
            let mut value = glib::Value::uninitialized();
            let ret = from_glib(ffi::gtk_style_provider_get_style_property(self.as_ref().to_glib_none().0, path.to_glib_none().0, state.into_glib(), pspec.as_ref().to_glib_none().0, value.to_glib_none_mut().0));
