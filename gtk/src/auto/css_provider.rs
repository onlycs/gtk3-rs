...
impl CssProvider {
    ...
    #[allow(clippy::should_implement_trait)]    pub fn default() -> Option<CssProvider> {
        assert_initialized_main_thread!();
        unsafe {
...
pub trait CssProviderExt: IsA<CssProvider> + sealed::Sealed + 'static {
    ...
    fn load_from_data(&self, data: &[u8]) -> Result<(), glib::Error> {
        ...
        unsafe {
            ...
            let is_ok = ffi::gtk_css_provider_load_from_data(self.as_ref().to_glib_none().0, data.to_glib_none().0, length, &mut error);
    ...
    fn load_from_file(&self, file: &impl IsA<gio::File>) -> Result<(), glib::Error> {
        unsafe {
            let mut error = std::ptr::null_mut();
            let is_ok = ffi::gtk_css_provider_load_from_file(self.as_ref().to_glib_none().0, file.as_ref().to_glib_none().0, &mut error);
            debug_assert_eq!(is_ok == glib::ffi::GFALSE, !error.is_null());
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    fn load_from_path(&self, path: &str) -> Result<(), glib::Error> {
        unsafe {
            let mut error = std::ptr::null_mut();
            let is_ok = ffi::gtk_css_provider_load_from_path(self.as_ref().to_glib_none().0, path.to_glib_none().0, &mut error);
            debug_assert_eq!(is_ok == glib::ffi::GFALSE, !error.is_null());
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    fn connect_parsing_error<F: Fn(&Self, &CssSection, &glib::Error) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn parsing_error_trampoline<P: IsA<CssProvider>, F: Fn(&P, &CssSection, &glib::Error) + 'static>(this: *mut ffi::GtkCssProvider, section: *mut ffi::GtkCssSection, error: *mut glib::ffi::GError, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(CssProvider::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(section), &from_glib_borrow(error))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"parsing-error\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(parsing_error_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
