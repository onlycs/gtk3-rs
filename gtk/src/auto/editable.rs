...
pub trait EditableExt: IsA<Editable> + sealed::Sealed + 'static {
    ...
    fn selection_bounds(&self) -> Option<(i32, i32)> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_editable_get_selection_bounds(self.as_ref().to_glib_none().0, start_pos.as_mut_ptr(), end_pos.as_mut_ptr()));
            if ret { Some((start_pos.assume_init(), end_pos.assume_init())) } else { None }
        }
    }
