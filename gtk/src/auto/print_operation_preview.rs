...
pub trait PrintOperationPreviewExt: IsA<PrintOperationPreview> + sealed::Sealed + 'static {
    ...
    fn connect_got_page_size<F: Fn(&Self, &PrintContext, &PageSetup) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn got_page_size_trampoline<P: IsA<PrintOperationPreview>, F: Fn(&P, &PrintContext, &PageSetup) + 'static>(this: *mut ffi::GtkPrintOperationPreview, context: *mut ffi::GtkPrintContext, page_setup: *mut ffi::GtkPageSetup, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(PrintOperationPreview::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(context), &from_glib_borrow(page_setup))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"got-page-size\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(got_page_size_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "ready")]
    fn connect_ready<F: Fn(&Self, &PrintContext) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn ready_trampoline<P: IsA<PrintOperationPreview>, F: Fn(&P, &PrintContext) + 'static>(this: *mut ffi::GtkPrintOperationPreview, context: *mut ffi::GtkPrintContext, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(PrintOperationPreview::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(context))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
