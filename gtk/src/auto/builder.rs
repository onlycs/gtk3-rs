...
pub trait BuilderExt: IsA<Builder> + sealed::Sealed + 'static {
    ...
    fn value_from_string(&self, pspec: impl AsRef<glib::ParamSpec>, string: &str) -> Result<glib::Value, glib::Error> {
        unsafe {
            let mut value = glib::Value::uninitialized();
            let mut error = std::ptr::null_mut();
            let is_ok = ffi::gtk_builder_value_from_string(self.as_ref().to_glib_none().0, pspec.as_ref().to_glib_none().0, string.to_glib_none().0, value.to_glib_none_mut().0, &mut error);
    ...
    fn value_from_string_type(&self, type_: glib::types::Type, string: &str) -> Result<glib::Value, glib::Error> {
        unsafe {
            let mut value = glib::Value::uninitialized();
            let mut error = std::ptr::null_mut();
            let is_ok = ffi::gtk_builder_value_from_string_type(self.as_ref().to_glib_none().0, type_.into_glib(), string.to_glib_none().0, value.to_glib_none_mut().0, &mut error);
            debug_assert_eq!(is_ok == glib::ffi::GFALSE, !error.is_null());
            if error.is_null() { Ok(value) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    fn connect_translation_domain_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_translation_domain_trampoline<P: IsA<Builder>, F: Fn(&P) + 'static>(this: *mut ffi::GtkBuilder, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::translation-domain\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(notify_translation_domain_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
