...
impl RecentInfo {
    ...
    pub fn create_app_info(&self, app_name: Option<&str>) -> Result<Option<gio::AppInfo>, glib::Error> {
        unsafe {
    ...
    pub fn application_info(&self, app_name: &str) -> Option<(glib::GString, u32, libc::c_long)> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_recent_info_get_application_info(self.to_glib_none().0, app_name.to_glib_none().0, &mut app_exec, count.as_mut_ptr(), time_.as_mut_ptr()));
            if ret { Some((from_glib_none(app_exec), count.assume_init(), time_.assume_init())) } else { None }
        }
    }
    ...
    pub fn applications(&self) -> Vec<glib::GString> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_full_num(ffi::gtk_recent_info_get_applications(self.to_glib_none().0, length.as_mut_ptr()), length.assume_init() as _);
            ret
        }
    }
    ...
    pub fn groups(&self) -> Vec<glib::GString> {
        unsafe {
            let mut length = std::mem::MaybeUninit::uninit();
            let ret = FromGlibContainer::from_glib_full_num(ffi::gtk_recent_info_get_groups(self.to_glib_none().0, length.as_mut_ptr()), length.assume_init() as _);
            ret
        }
    }
