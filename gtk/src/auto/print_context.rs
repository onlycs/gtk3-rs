...
impl PrintContext {
    ...
    pub fn hard_margins(&self) -> Option<(f64, f64, f64, f64)> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_print_context_get_hard_margins(self.to_glib_none().0, top.as_mut_ptr(), bottom.as_mut_ptr(), left.as_mut_ptr(), right.as_mut_ptr()));
            if ret { Some((top.assume_init(), bottom.assume_init(), left.assume_init(), right.assume_init())) } else { None }
        }
    }
