...
pub trait TreeModelExt: IsA<TreeModel> + sealed::Sealed + 'static {
    ...
    fn foreach<P: FnMut(&TreeModel, &TreePath, &TreeIter) -> bool>(&self, func: P) {
        ...
        unsafe extern "C" fn func_func<P: FnMut(&TreeModel, &TreePath, &TreeIter) -> bool>(model: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, iter: *mut ffi::GtkTreeIter, data: glib::ffi::gpointer) -> glib::ffi::gboolean {
    ...
    fn connect_row_changed<F: Fn(&Self, &TreePath, &TreeIter) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn row_changed_trampoline<P: IsA<TreeModel>, F: Fn(&P, &TreePath, &TreeIter) + 'static>(this: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, iter: *mut ffi::GtkTreeIter, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(TreeModel::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(path), &from_glib_borrow(iter))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"row-changed\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(row_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "row-deleted")]
    fn connect_row_deleted<F: Fn(&Self, &TreePath) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn row_deleted_trampoline<P: IsA<TreeModel>, F: Fn(&P, &TreePath) + 'static>(this: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(TreeModel::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(path))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"row-deleted\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(row_deleted_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
    ...
    fn connect_row_has_child_toggled<F: Fn(&Self, &TreePath, &TreeIter) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn row_has_child_toggled_trampoline<P: IsA<TreeModel>, F: Fn(&P, &TreePath, &TreeIter) + 'static>(this: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, iter: *mut ffi::GtkTreeIter, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(TreeModel::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(path), &from_glib_borrow(iter))
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"row-has-child-toggled\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(row_has_child_toggled_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "row-inserted")]
    fn connect_row_inserted<F: Fn(&Self, &TreePath, &TreeIter) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn row_inserted_trampoline<P: IsA<TreeModel>, F: Fn(&P, &TreePath, &TreeIter) + 'static>(this: *mut ffi::GtkTreeModel, path: *mut ffi::GtkTreePath, iter: *mut ffi::GtkTreeIter, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(TreeModel::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(path), &from_glib_borrow(iter))
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"row-inserted\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(row_inserted_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
