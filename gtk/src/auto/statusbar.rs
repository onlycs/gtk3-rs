...
pub trait StatusbarExt: IsA<Statusbar> + sealed::Sealed + 'static {
    ...
    fn connect_text_popped<F: Fn(&Self, u32, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn text_popped_trampoline<P: IsA<Statusbar>, F: Fn(&P, u32, &str) + 'static>(this: *mut ffi::GtkStatusbar, context_id: libc::c_uint, text: *mut libc::c_char, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(Statusbar::from_glib_borrow(this).unsafe_cast_ref(), context_id, &glib::GString::from_glib_borrow(text))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"text-popped\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(text_popped_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "text-pushed")]
    fn connect_text_pushed<F: Fn(&Self, u32, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn text_pushed_trampoline<P: IsA<Statusbar>, F: Fn(&P, u32, &str) + 'static>(this: *mut ffi::GtkStatusbar, context_id: libc::c_uint, text: *mut libc::c_char, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(Statusbar::from_glib_borrow(this).unsafe_cast_ref(), context_id, &glib::GString::from_glib_borrow(text))
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"text-pushed\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(text_pushed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
