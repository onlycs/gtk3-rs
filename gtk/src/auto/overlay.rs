...
pub trait OverlayExt: IsA<Overlay> + sealed::Sealed + 'static {
    ...
    fn reorder_overlay(&self, child: &impl IsA<Widget>, index_: i32) {
        unsafe {
            ffi::gtk_overlay_reorder_overlay(self.as_ref().to_glib_none().0, child.as_ref().to_glib_none().0, index_);
        }
    }
    ...
    fn set_overlay_pass_through(&self, widget: &impl IsA<Widget>, pass_through: bool) {
        unsafe {
            ffi::gtk_overlay_set_overlay_pass_through(self.as_ref().to_glib_none().0, widget.as_ref().to_glib_none().0, pass_through.into_glib());
        }
    }

    fn child_index<T: IsA<crate::Widget>>(&self, item: &T) -> i32 {
        crate::prelude::ContainerExtManual::child_property(self.as_ref(), &item.clone().upcast(),"index")
    }

    fn set_child_index<T: IsA<crate::Widget>>(&self, item: &T, index: i32) {
        crate::prelude::ContainerExtManual::child_set_property(self.as_ref(), &item.clone().upcast(),"index", &index)
    }
