...
pub trait AccelGroupExt: IsA<AccelGroup> + sealed::Sealed + 'static {
    ...
    fn activate(&self, accel_quark: glib::Quark, acceleratable: &impl IsA<glib::Object>, accel_key: u32, accel_mods: gdk::ModifierType) -> bool {
        unsafe {
            from_glib(ffi::gtk_accel_group_activate(self.as_ref().to_glib_none().0, accel_quark.into_glib(), acceleratable.as_ref().to_glib_none().0, accel_key, accel_mods.into_glib()))
        }
    }
    ...
    fn disconnect_key(&self, accel_key: u32, accel_mods: gdk::ModifierType) -> bool {
        unsafe {
            from_glib(ffi::gtk_accel_group_disconnect_key(self.as_ref().to_glib_none().0, accel_key, accel_mods.into_glib()))
        }
    }
    ...
    fn connect_accel_activate<F: Fn(&Self, &glib::Object, u32, gdk::ModifierType) -> bool + 'static>(&self, detail: Option<&str>, f: F) -> SignalHandlerId {
        unsafe extern "C" fn accel_activate_trampoline<P: IsA<AccelGroup>, F: Fn(&P, &glib::Object, u32, gdk::ModifierType) -> bool + 'static>(this: *mut ffi::GtkAccelGroup, acceleratable: *mut glib::gobject_ffi::GObject, keyval: libc::c_uint, modifier: gdk::ffi::GdkModifierType, f: glib::ffi::gpointer) -> glib::ffi::gboolean {
            let f: &F = &*(f as *const F);
            f(AccelGroup::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(acceleratable), keyval, from_glib(modifier)).into_glib()
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            let detailed_signal_name = detail.map(|name| { format!("accel-activate::{name}\0") });
            let signal_name: &[u8] = detailed_signal_name.as_ref().map_or(&b"accel-activate\0"[..], |n| n.as_bytes());
            connect_raw(self.as_ptr() as *mut _, signal_name.as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(accel_activate_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "accel-changed")]
    fn connect_accel_changed<F: Fn(&Self, u32, gdk::ModifierType, &glib::Closure) + 'static>(&self, detail: Option<&str>, f: F) -> SignalHandlerId {
        unsafe extern "C" fn accel_changed_trampoline<P: IsA<AccelGroup>, F: Fn(&P, u32, gdk::ModifierType, &glib::Closure) + 'static>(this: *mut ffi::GtkAccelGroup, keyval: libc::c_uint, modifier: gdk::ffi::GdkModifierType, accel_closure: *mut glib::gobject_ffi::GClosure, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(AccelGroup::from_glib_borrow(this).unsafe_cast_ref(), keyval, from_glib(modifier), &from_glib_borrow(accel_closure))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            let detailed_signal_name = detail.map(|name| { format!("accel-changed::{name}\0") });
            let signal_name: &[u8] = detailed_signal_name.as_ref().map_or(&b"accel-changed\0"[..], |n| n.as_bytes());
            connect_raw(self.as_ptr() as *mut _, signal_name.as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(accel_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "is-locked")]
    fn connect_is_locked_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_is_locked_trampoline<P: IsA<AccelGroup>, F: Fn(&P) + 'static>(this: *mut ffi::GtkAccelGroup, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
    ...
    fn connect_modifier_mask_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_modifier_mask_trampoline<P: IsA<AccelGroup>, F: Fn(&P) + 'static>(this: *mut ffi::GtkAccelGroup, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(AccelGroup::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::modifier-mask\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(notify_modifier_mask_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
