...
impl GestureSwipe {
    ...
    pub fn velocity(&self) -> Option<(f64, f64)> {
        unsafe {
            ...
            let ret = from_glib(ffi::gtk_gesture_swipe_get_velocity(self.to_glib_none().0, velocity_x.as_mut_ptr(), velocity_y.as_mut_ptr()));
            if ret { Some((velocity_x.assume_init(), velocity_y.assume_init())) } else { None }
        }
    }

    #[doc(alias = "swipe")]
    pub fn connect_swipe<F: Fn(&Self, f64, f64) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn swipe_trampoline<F: Fn(&GestureSwipe, f64, f64) + 'static>(this: *mut ffi::GtkGestureSwipe, velocity_x: libc::c_double, velocity_y: libc::c_double, f: glib::ffi::gpointer) {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"swipe\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(swipe_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }
}
